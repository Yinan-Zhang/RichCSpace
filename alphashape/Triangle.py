...
class Triangle:
	...
	def radical_center(self):
		...
		i_vec = (self.spheres[1].center - self.spheres[0].center).normalize();
		j_vec = (self.spheres[2].center - self.spheres[0].center).normalize();
		y_ = inter_y / sin_a;
		x_ = inter_x - y_ * cos_a;
		radical_center = (  i_vec * x_ + j_vec * y_ ) + self.spheres[0].center;
		return radical_center;

	def is_semi_filled( self ):
		if not self.is_filled():
			return self.spheres[0].intersects(self.spheres[1]) and self.spheres[0].intersects(self.spheres[2]) and self.spheres[2].intersects(self.spheres[1])
		return False;
...
class Triangulator:
	...
	def triangulate_raw( self ):
		'''get row data about triangulation. return all triangles'''
		#######################################
